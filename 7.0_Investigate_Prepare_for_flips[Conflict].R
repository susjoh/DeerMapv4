...
mapdata <- read.table(paste0("results/6_Linkage_Map_Positions_run5_", AnalysisSuffix, ".txt"),
                      header = T, stringsAsFactors = F)
...
head(mapdata)
table(mapdata$analysisID)
mapdata$analysisID[mapdata$CEL.LG != 34] <-  paste0(mapdata$analysisID[mapdata$CEL.LG != 34], "_dbx")
...
save(famped, mapdata, skel.map, abeldata, AnalysisSuffix,
     AnalysisSuffix2, AnalysisSuffix3, lg.vec, file = "eddie/6_run5_Deer_Data_for_Eddie_a.RData")
