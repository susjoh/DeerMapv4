...
# if(parseFlipsRun == TRUE){
  ...
  write.table(flips.final    , "results/8_Full_Flips_Output_NegLi.txt", row.names = F, sep = "\t", quote = F)
  write.table(map.stats.final, "results/8_Map_Statistics_per_Flips_Run.txt", row.names = F, sep = "\t", quote = F)
  write.table(snp.order.final, "results/8_SNP_Order_per_Flips_Run.txt", row.names = F, sep = "\t", quote = F)
