...
fullsumm <- subset(fullsumm, Dad.Geno != Mum.Geno)
x.dad <- subset(fullsumm, Dad.Geno == "A/B" & Mum.Geno != "A/B")
head(x.dad)

dadsumm <- data.frame(Dad.A.Count = tapply(x.dad$Dad.A, x.dad$SNP.Name, sum),
