...
mapdata <- read.table("results/8_Linkage_Map_Positions_CEL_run5_a.txt", header = T, stringsAsFactors = F)
head(mapdata)
...
write.table(max.map, "results/8_Predicted_Physical_Size_run5_a.txt", row.names = F, sep = "\t", quote = F)

write.table(mapdata, "results/8_Linkage_Map_Positions_CEL_run5_dumpos_a.txt", row.names = F, sep = "\t", quote = F)
