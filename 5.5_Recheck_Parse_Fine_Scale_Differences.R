...
for(i in 1:nrow(map.chunk.50)){
  ...
  invmap <- read.table(paste0("finescale/chr", map.chunk.50$CEL.LG[i], AnalysisSuffix, "_cel_ckP_Re_", map.chunk.50$chunk[i], "_inv.parsemap"), header = T, stringsAsFactors = F)
  delmap <- read.table(paste0("finescale/chr", map.chunk.50$CEL.LG[i], AnalysisSuffix, "_cel_ckP_Re_", map.chunk.50$chunk[i], "_del.parsemap"), header = T, stringsAsFactors = F)
  
  map.chunk.50$Inversion.Len[i] <- invmap$cMPosition[nrow(invmap)]
  map.chunk.50$Initial.Len[i] <- test$cMPosition.run4[nrow(test)]
...
write.table(map.chunk.50, paste("results/5_Recheck_MapChunk_DelInvCheck_Summary_", AnalysisSuffix2, ".txt"),
            row.names = F, quote = F)

write.table(chunk.res, paste("results/5_Recheck_MapChunk_DelInvCheck_Full_", AnalysisSuffix2, ".txt"),
            row.names = F, quote = F)
...
map.chunk.50 <- read.table(paste("results/5_Recheck_MapChunk_DelInvCheck_Summary_", AnalysisSuffix2, ".txt"),
...
chunk.res <- read.table(paste("results/5_Recheck_MapChunk_DelInvCheck_Full_", AnalysisSuffix2, ".txt"),
                        header = T, stringsAsFactors = F)
...
chunk.res <- join(chunk.res, mapdata[,c("SNP.Name", "chunk", "cMPosition.run4", "Order")])
...
edit.tab <- rbind(cbind(map.chunk.50[which(map.chunk.50$Freq < 10 & map.chunk.50$Deletion.Diff > 1),], edit = "del"))

edit.tab <- subset(edit.tab, CEL.LG == 34)
