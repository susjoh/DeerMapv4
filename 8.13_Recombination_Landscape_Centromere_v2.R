...
ggplot(subset(bin.tab.sex.acro, Fission == "A_Fission_Old_Centro"),
       aes(x = Window, y = Recomb.Rate, colour = factor(Sex))) +
  #geom_line() +
  stat_smooth(se = F, size = 0.5, span = 0.5) +
  facet_grid(Sex~CEL.LG) +
  scale_colour_brewer(palette = "Set1") +
  theme(axis.text.x  = element_text (size = 12),
        axis.text.y  = element_text (size = 12),
        strip.text.x = element_text (size = 12),
...
summary(fit1 <- gam(Recomb.Rate ~ s(Window, by = factor(Fission), k = 10) + s(I(1/Est.Length)), data = bin.tab.sex.acro.f))
